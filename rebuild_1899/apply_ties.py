...
ties = []
for tag in ("E", "F"):
    for t in json.load(open(os.path.join(OUT, f"adjudicate_{tag}_result.json")))["ties"]:
...
for sweep in range(6):
    ...
    for uid in sorted(FLAGGED - set(placed_now)):
        ...
        if fit:
            ...
            votes = [g - M @ m for m, g in cor]
            t_ = np.median(np.array(votes), axis=0)
            sp = float(np.abs(np.array(votes) - t_).max()) if len(votes) > 1 else 0.0
            if sp > 120:
                print(f"  {uid}: tie votes disagree by {sp:.0f}px — left flagged")
                continue
            how = f"ties-trans(n={len(cor)},spread={sp:.0f})"
